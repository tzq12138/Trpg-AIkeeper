...
> **统计**：21 个 HTML 页面 · 47 张预览截图 · 5 份技术方案 MD · 1 份设计规范
> **整理状态**：✅ 已完成语义化重命名 + 去重（2026-06-27）
...
│
├── host_dashboard_v1/  ~ host_logs/      ← 守密人端（Host/Keeper）7 个页面
├── player_status_basic/  ~ player_map_2b_v2/  ← 玩家端（Investigator）14 个页面
```

> **备份**：整理前的原始目录已备份至 `stitch_host_player_frontend_interface_backup_20260627/`，含全部 50 个原始文件。
...
如需恢复整理前的原始结构，完整备份位于：
```
stitch_host_player_frontend_interface_backup_20260627/
```
该备份包含全部 50 个原始文件（含已删除的 `host_logs_2` 及原始 `_1`~`_13` 命名）。

---

*本索引由 TechnicalArtist 整理 · 2026-06-27 完成语义化重命名与去重*

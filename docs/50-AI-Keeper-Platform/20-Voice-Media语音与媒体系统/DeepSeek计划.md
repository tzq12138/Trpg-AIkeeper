# Voice / Media 语音与媒体系统 DeepSeek 计划 V2.0
...
- 明确 STT endpoint 是否能被测试 client 调用。
...
- mock provider 返回 `transcribedText`。
- 超限和鉴权错误使用预期状态码。
- 请求结束后无长期音频文件。
...
- 让 BGM/SFX/video/image 引用统一走 Asset 的安全 URL 或 asset id。
...
- 氛围事件引用 asset id 时，服务端能解析成受控 URL。
- 未公开素材不会被发给 Player。
...
7. Projection 发送 `s2c_atmosphere`，HostStage 更新视觉氛围。

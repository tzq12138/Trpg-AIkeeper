...
class Engine:
    ...
    def bump_state_version(self, room_id: str) -> dict:
        ...
        self.conn.commit()
        logger.debug("bump_state_version: room=%s %s→%s", room_id, base, base + 1)
    ...
    def complete_action(
        ...
        self.conn.commit()
        logger.info("complete_action: action_id=%s status=%s room=%s delayed=%s",
                    action_id, status, action["room_id"] if action else "?", len(delayed))
        return delayed
